...
static void query(void)
{
    ...
    gimp_install_procedure(PLUG_IN_PROC,
        _("Clean blur, Deblur, Remove blur"),
        _("More_Deblur_Help"),
        "Dell Du <18588220928@163.com>",
        "Dell Du",
        "2020-2024", _("_Blur_"), "RGB*, GRAY*", GIMP_PLUGIN, G_N_ELEMENTS(args), 0, args, NULL);
...
static GimpPDBStatusType start_image_deblur(gint32 drawable_id)
{
    ...
    recv_image = vision_image_service((char*)"image_deblur", send_image, NULL);
...
run(const gchar* name, gint nparams, const GimpParam* param, gint* nreturn_vals, GimpParam** return_vals)
{
    ...
    *return_vals = values;
    ...
    values[0].data.d_status = start_image_deblur(drawable_id);
