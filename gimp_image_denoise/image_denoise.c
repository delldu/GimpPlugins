...
static void query(void)
{
    ...
    gimp_install_procedure(PLUG_IN_PROC,
        _("Clean noise, Denoise, Remove noise"),
        _("More_Denoise_Help"),
...
static GimpPDBStatusType start_image_denoise(gint32 drawable_id)
{
    ...
    recv_image = vision_image_service((char*)"image_denoise", send_image, NULL);
...
run(const gchar* name, gint nparams, const GimpParam* param, gint* nreturn_vals, GimpParam** return_vals)
{
    ...
    *return_vals = values;
    ...
    values[0].data.d_status = start_image_denoise(drawable_id);
