...
static IMAGE *dereflection_rpc_service(IMAGE * send_image)
{
	return normal_service(AI_TASKSET, "image_dereflection", send_image, NULL);
}
...
static GimpPDBStatusType start_image_dereflection(gint32 drawable_id)
{
	...
	if (image_valid(send_image)) {
		recv_image = dereflection_rpc_service(send_image);
		...
		if (image_valid(recv_image)) {
			...
			g_message("Error: Dereflection service not available.\n");
		}
		image_destroy(send_image);
	} else {
		status = GIMP_PDB_EXECUTION_ERROR;
		g_message("Error: Dereflection source.\n");
	}
