...
static IMAGE *dereflection_rpc_service(int id, IMAGE * send_image)
{
	return normal_service(AI_TASKSET, "image_dereflection", id, send_image, NULL);
}
...
static GimpPDBStatusType start_image_dereflection(gint32 drawable_id)
{
	...
	if (file_exist(output_file)) {
		...
		if (image_valid(send_image)) {
			recv_image = dereflection_rpc_service(drawable_id, send_image);
