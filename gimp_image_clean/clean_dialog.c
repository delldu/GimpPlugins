...
gboolean clean_dialog()
{
	...
	combo = gimp_int_combo_box_new(
				"Deep Learning", IMAGE_CLEAN_SERVICE,
				"CUDA BM3d", IMAGE_CLEAN_SERVICE_WITH_BM3D,
				"Guide Filter", IMAGE_CLEAN_SERVICE_WITH_GUIDED_FILTER,
				NULL);
	gimp_int_combo_box_set_active(GIMP_INT_COMBO_BOX(combo), IMAGE_CLEAN_SERVICE);
	...
	adj = gimp_scale_entry_new(GTK_TABLE(table), 0, 1,
		...
		clean_options.strength, 0, 100 /*[lo, up]*/, 1, 10 /*step*/, 0, TRUE, 0, 0, "Clean Strength", NULL);
	...
	if (run) {
		CheckPoint("clean_options.method = 0x%x, clean_options.strength = %d", clean_options.method, clean_options.strength);
	}
