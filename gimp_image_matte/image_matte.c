...
static GimpPDBStatusType start_image_matte(gint drawable_id)
{
	...
	if (image_valid(send_image)) {
		...
		image_ai_cache_filename("image_matte_output", sizeof(output_file), output_file);
		if (is_same_image_hash(image_hash.input, hash) && file_exist(output_file)) {
			recv_image =  image_load(output_file);
		} else {
			recv_image = normal_service("image_matte", send_image, NULL, output_file);
		}
...
static void query(void)
{
	...
	gimp_install_procedure(PLUG_IN_PROC,
						   ...
						   "Dell Du",
						   "2020-2022",
						   _("Photo Matte"), "RGB*, GRAY*", GIMP_PLUGIN, G_N_ELEMENTS(args), 0, args, NULL);
