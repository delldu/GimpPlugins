...
static GimpPDBStatusType start_image_artist_style(gint32 drawable_id, gint32 style_drawable_id)
{
    ...
    recv_image = vision_style_service("image_artist_style", send_image, style_image);
    image_destroy(send_image);
    image_destroy(style_image);
    vision_save_image_to_gimp(recv_image, (char*)"artist_style");
...
run(const gchar* name, gint nparams, const GimpParam* param, gint* nreturn_vals, GimpParam** return_vals)
{
    ...
    *return_vals = values;
    ...
    if (style_drawable_id < 0) {
        ...
        // gimp_layer_set_opacity(style_drawable_id, 100.0);
    ...
    // update ...
    values[0].data.d_status = start_image_artist_style(drawable_id, style_drawable_id);
    if (run_mode != GIMP_RUN_NONINTERACTIVE)
        gimp_displays_flush();
