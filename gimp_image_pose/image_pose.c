...
#define PLUG_IN_PROC "gimp_image_pose"
...
static void query(void)
{
    ...
    gimp_install_procedure(PLUG_IN_PROC,
        ...
        "Dell Du",
        "2020-2024",
        _("Pose"), "RGB*, GRAY*", GIMP_PLUGIN, G_N_ELEMENTS(args), 0, args, NULL);

    gimp_plugin_menu_register(PLUG_IN_PROC, "<Image>/AI/Detect/");
}

static GimpPDBStatusType start_image_pose(gint32 drawable_id)
{
    gint channels;
    GeglRectangle rect;
    IMAGE *send_image, *recv_image;

    gimp_progress_init("Detect Pose ...");
...
run(const gchar* name, gint nparams, const GimpParam* param, gint* nreturn_vals, GimpParam** return_vals)
{
    ...
    *return_vals = values;
    ...
    values[0].data.d_status = start_image_pose(drawable_id);
