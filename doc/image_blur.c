...
static void process_row(guchar ** row,
						guchar * outrow, gint x1, gint y1, gint width, gint height, gint channels, gint i);
static void shuffle(GimpPixelRgn * rgn_in, guchar ** row, gint x1, gint y1, gint width, gint height, gint ypos);
...
static void query(void)
{
	...
	gimp_install_procedure("plug-in-myblur5",
						   "My blur 5 (preview)",
						   "Blurs the image",
						   "David Neary",
						   "Copyright David Neary",
						   "2004",
						   "_My blur 5 (preview)...", "RGB*, GRAY*", GIMP_PLUGIN, G_N_ELEMENTS(args), 0, args, NULL);

	gimp_plugin_menu_register("plug-in-myblur5", "<Image>/Filters/Blur");
}
...
static void blur(GimpDrawable * drawable, GimpPreview * preview)
{
	...
	for (ii = -bvals.radius; ii <= bvals.radius; ii++) {
		gimp_pixel_rgn_get_row(&rgn_in, row[bvals.radius + ii], x1, y1 + CLAMP(ii, 0, height - 1), width);
	}

	for (i = 0; i < height; i++) {
		/* To be done for each tile row */
		process_row(row, outrow, x1, y1, width, height, channels, i);
...
static void
process_row(guchar ** row, guchar * outrow, gint x1, gint y1, gint width, gint height, gint channels, gint i)
{
	gint j;

	for (j = 0; j < width; j++) {
		gint k, ii, jj;
		gint left = (j - bvals.radius), right = (j + bvals.radius);
		...
		for (k = 0; k < channels; k++) {
			...
			outrow[channels * j + k] = sum / (4 * bvals.radius * bvals.radius + 4 * bvals.radius + 1);
		}
	}
}
...
static void shuffle(GimpPixelRgn * rgn_in, guchar ** row, gint x1, gint y1, gint width, gint height, gint ypos)
{
	...
	gimp_pixel_rgn_get_row(rgn_in, row[0], x1, MIN(ypos + bvals.radius + y1, y1 + height - 1), width);
...
static gboolean blur_dialog(GimpDrawable * drawable)
{
	...
	dialog = gimp_dialog_new("My blur---My blur---My blur", "myblur",
							 NULL, 0,
	...
	// spinbutton = gimp_spin_button_new(&spinbutton_adj, bvals.radius, 1, 32, 1, 1, 1, 5, 0);
	...
	g_signal_connect_swapped(preview, "invalidated", G_CALLBACK(blur), drawable);
	...
	// g_signal_connect(spinbutton_adj, "value_changed", G_CALLBACK(gimp_int_adjustment_update), &bvals.radius);
