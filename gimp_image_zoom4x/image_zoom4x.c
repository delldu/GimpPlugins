...
static void query(void)
{
    ...
    gimp_install_procedure(PLUG_IN_PROC,
        _("4X Super Resolution"),
        _("More_Zoom4x_Help"),
        "Dell Du <18588220928@163.com>",
        "Dell Du",
        "2020-2023", _("4X"), "RGB*, GRAY*", GIMP_PLUGIN, G_N_ELEMENTS(args), 0, args, NULL);
...
static GimpPDBStatusType start_image_zoom4x(gint32 drawable_id)
{
    ...
    if (image_valid(send_image)) {
        recv_image = vision_image_service("image_zoom4x", send_image, NULL);
    ...
    if (status == GIMP_PDB_SUCCESS && image_valid(recv_image)) {
        vision_save_image_to_gimp(recv_image, (char*)"zoom4x");
