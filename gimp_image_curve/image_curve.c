...
static void query(void)
{
	...
	gimp_install_procedure(PLUG_IN_PROC,
						   ...
						   "Dell Du",
						   "2020-2022",
						   _("Deep Color Curves"), "RGB*, GRAY*", GIMP_PLUGIN, G_N_ELEMENTS(args), 0, args, NULL);
...
static GimpPDBStatusType start_image_curve(gint32 drawable_id)
{
	...
	if (image_valid(send_image)) {
		...
		image_ai_cache_filename("image_curve_output", sizeof(output_file), output_file);
		if (is_same_image_hash(image_hash.input, hash) && file_exist(output_file)) {
			recv_image =  image_load(output_file);
		} else {
			recv_image = normal_service("image_curve", send_image, NULL, output_file);
		}
	...
	if (image_valid(recv_image)) {
		...
		status = GIMP_PDB_EXECUTION_ERROR;
		g_message("Curve service not available.\n");
	}
